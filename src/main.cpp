    ...
    int32_t cpu_family = 0; // We use this to determine if a CPU is "modern" enough.
        ...
        if (cpuid_present() && cpuid_get_raw_data(&cpu_raw) >= 0)
        {
            if (cpu_identify(&cpu_raw, &cpu_data) >= 0)
            {
                ...
                SYNCMSG("CPU %s, family %d, model %d, architecture %d, cpu_codename %s", cpu_data.vendor_str,
                    cpu_data.family, cpu_data.model, cpu_data.architecture, cpu_data.cpu_codename);
